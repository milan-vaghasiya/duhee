				...
				<div class="card">
					<div class="card-header">
						<div class="row">
							<div class="col-md-4">
								...
								<a href="<?= base_url($headData->controller . "/indexReturn") ?>" class="btn waves-effect waves-light btn-outline-primary permission-write">Return</a>
                            </div>
							<div class="col-md-4">
					...
					<div class="card-body">
						...
						<div class="table-responsive">
							<table id='jobWorkTable' class="table table-bordered ssTable ssTable-cf" data-ninput='[0,-1]' data-url='/getDTRows'></table>
						</div>
					</div>
				</div>
			</div>
...
<script src="<?php echo base_url(); ?>assets/js/custom/eway-bill.js?v=<?= time() ?>"></script>
<script>
$(document).ready(function() {
	...
	$(document).on('change', "#job_order_trans_id", function() {
		var trans_id = $(this).val();
			var trans_ref_id = $('#job_order_trans_id :selected').data('ref_id');			
			$("#trans_ref_id").val(trans_ref_id); 		
	...
	$(document).on('click', '.addRow', function() {
		...
		var trans_ref_id = $("#trans_ref_id").val();
		var qty = $("#qty").val();
		...
		if(IsValid){
			...
			var process_name=$('#job_order_trans_id :selected').data('process_name');
			var post = {
				id: "",
				qty: qty,
...
function addRow(data) {
	...
	var transRefInput = $("<input/>",{type:"hidden",name:"trans_ref_id[]",value:data.trans_ref_id});
...
function approveJobWorkReturn(id,val,name='Record'){
...
function approveReturn(id,name='Record'){
	...
	$.confirm({
		title: 'Confirm!',
		content: 'Are you sure want to Approve this '+name+'?',
		type: 'red',
		buttons: {   
			ok: {
				text: "ok!",
				...
				action: function(){
					$.ajax({
						url: base_url + controller + '/approveReturn',
						data: send_data,
						type: "POST",
						dataType:"json",
						success:function(data)
...
function rejectReturn(id,name='Record'){
	var send_data = { id:id };
	$.confirm({
		title: 'Confirm!',
		...
		buttons: {   
			ok: {
				...
				action: function(){
					$.ajax({
						url: base_url + controller + '/rejectReturn',
						data: send_data,
						type: "POST",
						dataType:"json",
						success:function(data)
