			...
			<div class="col-12">
				<div class="card">
					<div class="card-header text-center">
						<h4><u>JobWork Scrap Invoice</u></h4>
					</div>
					<div class="card-body">
						<form autocomplete="off" id="saveSalesInvoice">
							<div class="col-md-12">
								...
								<input type="hidden" name="entry_type" id="entry_type" value="<?= (!empty($invoiceData->entry_type)) ? $invoiceData->entry_type : "20" ?>">
								...
								<input type="hidden" name="gross_weight" id="gross_weight" value="<?= (!empty($invoiceData->gross_weight)) ? $invoiceData->gross_weight : '' ?>" />
								
								<input type="hidden" name="total_packet" id="total_packet" value="<?= (!empty($invoiceData->total_packet)) ? $invoiceData->total_packet : '' ?>"/>
								
								<input type="hidden" name="supply_place" id="supply_place" value="<?= (!empty($invoiceData->supply_place)) ? $invoiceData->supply_place : "" ?>"/>
								...
								<div class="row form-group">
									...
									<div class="col-md-5">
										...
										<div for="party_id1" class="float-right">
											<a href="javascript:void(0)" class="text-primary font-bold createScrapInvoice permission-write1" datatip="Scrap" flow="down">+Scrap</a>
										</div>
									...
									<div class="col-md-3 form-group">
										...
										<input type="text" name="gstin" id="gstin" class="form-control" placeholder="Enter Gst No." value="<?= (!empty($invoiceData->gstin)) ? $invoiceData->gstin :((!empty($invMaster->gstin))?$invMaster->gstin:"")?>" />
											...
											<tbody id="tempItem" class="temp_item">
												...
												?>
														<tr>
															...
															<td>
																<?= $row->qty ?>
																<input type="hidden" id="qty<?=$row->id?>" name="qty[]" value="<?= $row->qty ?>">
															...
															<td>
																<!-- <?= $row->price ?> -->
																<input type="number" name="price[]" id="price<?=$i?>" class="form-control floatOnly " data-id="<?=$row->id?>" value="<?= $row->price ?>" onkeyup='calculateAmt(<?= json_encode($row) ?>,this,<?=$i?>);' data-fn="save_close">
															...
															<td>
																...
																<input type="number" name="disc_per[]" id="disc_per<?=$i?>" onkeyup='calculateAmt(<?= json_encode($row) ?>,this,<?=$i?>);' class="form-control " data-id="<?=$row->id?>" value="<?= $row->disc_per ?>">
																<input type="hidden" name="disc_amt[]" id="disc_amt<?=$row->id?>" value="<?= $row->disc_amount ?>">
															</td>
															<td class="amountCol">
																<?= $row->taxable_amount ?>
																<input type="hidden"  name="amount[]" id="amount<?=$row->id?>" value="<?= $row->taxable_amount ?>">
															</td>
															<td class="netAmtCol">
																<?= $row->net_amount ?>
																<input type="hidden" name="net_amount[]" id="net_amount<?=$row->id?>" value="<?= $row->net_amount ?>">
															</td>
															<td>
																<!-- <?= $row->item_remark ?> -->
																<input type="text" name="item_remark[]" id="itemRemark<?=$i?>" class="form-control " value="<?= $row->item_remark ?>" onkeyup='calculateAmt(<?= json_encode($row) ?>,this,<?=$i?>);'>
															</td>
							...
							<div class="col-md-12 form-group">
								...
								<span class="dropdown float-right">
									...
									<div class="dropdown-menu dropdown-menu-left user-dd animated flipInY" x-placement="start-left">
										...
										<a class="dropdown-item leadActionStatic addNewMaster" href="javascript:void(0)" data-button="both" data-modal_id="modal-lg" data-function="addProduct/1" data-controller="products" data-class_name="itemMasterOptions" data-form_title="Add Product"> + Product</a>

									</div>
								</span>
								<div class="input-group">
									<input type="text" id="item_name_dis" class="form-control" value="" readonly style="width: 95%;" /> 
									<button type="button" class="btn btn-outline-primary" onclick="searchFGItems();" style="width: 5%;"><i class="fa fa-plus"></i></button>
								</div>
							...
							<table id="orderTable" class="table table-bordered">
								<thead class="thead-info">
									<tr>
										...
										<th class="text-center">Challn Date</th>
...
<script src="<?php echo base_url(); ?>assets/js/custom/jobwork-scrap-invoice.js?v=<?= time() ?>"></script>
...
if (!empty($invItems)) {
	foreach ($invItems as $row) :
		...
		if (!empty($row->scrap_weight)) :
			...
			$row->qty = $row->scrap_weight;
			$row->price = $row->scarp_rate_pcs;
			...
			$row->cgst_amt = round(($row->amount * $row->cgst_per) / 100, 2);
			$row->sgst_amt = round(($row->amount * $row->sgst_per) / 100, 2);
