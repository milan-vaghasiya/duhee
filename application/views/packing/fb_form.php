<form>
    <div class="col-md-12">
        <div class="row">
            ...
            <?php
            $prefix='';
            if($packing_type == 1){$prefix='PFB';}else{$prefix="PLB";}
            ?>
            <div class="col-md-2 form-group">
                <label for="trans_number">FB No</label>
                <input type="text" name="trans_number" id="trans_number" class="form-control" readonly value="<?= !empty($dataRow->trans_number) ? $dataRow->trans_number : sprintf($prefix."%03d", $trans_no) ?>">
            ...
            <div class="col-md-6">
                <div class="table-responsive">
                    <table class="table  table-bordered item-list-bb">
                        <thead class="thead-info">
                            <tr >
                                <th colspan="4" class="text-center">Live FIR Qty</th>
                            </tr>
                            <tr >
                                <th></th>
                                <th>FIR No</th>
                        ...
                        <tbody id="firData">
                            <?php
                            if (!empty($stockData['liveFIRHtml'])) {
                                echo $stockData['liveFIRHtml'];
                            } else { ?>
                                <tr> <th colspan="5" class="text-center">No data available.</th> </tr>
                            <?php } ?>
                        </tbody> 
...
<script>
    $(document).ready(function() {
        ...
        $(document).on('keyup','.calQtyPerBox',function(){
            var qtyPerBoxArr=[];
            qtyPerBoxArr = $('.boxQty').map(function(){ return $(this).val();}).get();
