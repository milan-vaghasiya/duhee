				...
				<div class="card">
						...
						<div class="row">
							<div class="col-md-9">
							    <?php if(!empty($mType) && $mType != '6~7'){ ?>
                                    <a href="<?=base_url($headData->controller."/index")?>" class="btn waves-effect waves-light btn-outline-primary permission-write active"> Pending</a>
                                    ...
                                    <a href="<?=base_url($headData->controller."/index3")?>" class="btn waves-effect waves-light btn-outline-primary  permission-write"> Material Issued</a>
                                <?php }else{ ?>
                                    <a href="<?=base_url($headData->controller."/qcIndex")?>" class="btn waves-effect waves-light btn-outline-primary permission-write active"> Pending</a>
                                    ...
                                    <a href="<?=base_url($headData->controller."/qcIndex3")?>" class="btn waves-effect waves-light btn-outline-primary  permission-write"> Material Issued</a>
                                <?php } ?>
							</div>

							<div class="col-md-3 float-right">
								<select class="form-control" id="statusTabChange">
									<option value="0">All</option>
									<option value="1">Due Isuue</option>
									<option value="2">Pending Allocation</option>
									<option value="3">Pending Issue</option>
								</select>
							</div>
					...
					<div class="card-body">
						<div class="table-responsive">
							<table id='issueMaterialTable' class="table table-bordered ssTable ssTable-cf" data-ninput='[0]' data-url='/getDTRows/<?= $mType ?>'></table>
						</div>
					</div>
				</div>
			</div>
...
<script>
	$(document).ready(function() {
		...
		$(document).on('change',"#statusTabChange",function(){
			var status=$(this).val();
			$("#issueMaterialTable").attr("data-url",'/getDTRows/'+<?= '"'.$mType.'"' ?> +'/'+status);
	...
	function dispatch(data) {
		...
		$.ajax({
			...
			$("#" + data.modal_id + " .modal-footer .btn-save").attr('onclick', "saveIssueMaterial('" + data.form_id + "');");
			if (button == "close") {
	...
	function saveIssueMaterial(formId,fnsave){
		setPlaceHolder();
		...
		$.ajax({
			...
			if(data.status===0){
				...
				var url = base_url + controller + '/printToolLife/' + data.issue_id;
