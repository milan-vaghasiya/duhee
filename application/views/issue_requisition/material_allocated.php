				...
				<div class="card">
						...
						<div class="row">
							<div class="col-md-9">
								<?php if(!empty($mType) && $mType != '6~7'){ ?>
                                    ...
                                    <a href="<?=base_url($headData->controller."/index2")?>" class="btn waves-effect waves-light btn-outline-primary  permission-write active"> Material Allocated</a>
                                ...
                                <?php }else{ ?>
                                    ...
                                    <a href="<?=base_url($headData->controller."/qcIndex2")?>" class="btn waves-effect waves-light btn-outline-primary  permission-write active"> Material Allocated</a>
							...
							<div class="col-md-3 float-right">
								<select class="form-control" id="statusTabChange">
									...
									<option value="1">Due Issue</option>
									<option value="2">Pending Issue</option>
								</select>
							</div>
					...
					<div class="card-body">
						<div class="table-responsive">
							<table id='mtrlAllocTable' class="table table-bordered ssTable ssTable-cf" data-ninput='[0]' data-url='/getDTAllocRows/<?= $mType ?>'></table>
						</div>
					</div>
				</div>
			</div>
...
<script>
	$(document).ready(function() {
		...
		$(document).on('change', "#statusTabChange", function() {
			var status = $(this).val();
			$("#mtrlAllocTable").attr("data-url", '/getDTAllocRows/' + <?= '"'.$mType.'"' ?> + '/' + status);
			ssTable.state.clear();
			initTable();
		});
	});
