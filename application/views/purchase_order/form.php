            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header text-center">
                        <h4><u><?=(!empty($order_type)?'RM Purchase Order':'Purchase Order')?></u></h4>
                    </div>
                    <div class="card-body">
                        <form autocomplete="off" id="savePurchaseOrder">
                        	<div class="col-md-12">
								...
								<input type="hidden" name="po_prefix" id="po_prefix" class="po_prefix" value="<?=(!empty($dataRow->po_prefix))?$dataRow->po_prefix:$po_prefix?>" />
								
								<input type="hidden" name="order_type" id="order_type" value="<?=(!empty($order_type))?$order_type:0?>" />
								
								<div class="row form-group">
									<div class="col-md-3">
                                        <label for="enq_no">PO No.</label>
                                        <div class="input-group mb-3">
                                            <input type="text" name="po_prefix" id="po_prefix"  class="form-control po_prefix" value="<?=(!empty($dataRow->po_prefix))?$dataRow->po_prefix:$po_prefix?>" readonly />
												...
												<?php
													...
													if (!empty($reqItemList)) :
														...
														foreach ($reqItemList as $reqItem) :
															...
															$reqItem->amount = round(($reqItem->req_qty * $reqItem->price), 2);
													...
													?>
															<tr>
																...
																<td>
																	<?= ($reqItem->full_name) ?>
																	<input type="hidden" name="item_id[]" value="<?= $reqItem->req_item_id ?>">
																...
																<td>
																	<?= $reqItem->req_qty ?>
																	<input type="hidden" name="qty[]" value="<?= $reqItem->req_qty ?>">
																</td>
																<td>
																	<?= $reqItem->unit_name ?>
																	<input type="hidden" name="unit_id[]" value="<?= $reqItem->unit_id ?>">
    																<input type="hidden" name="fgitem_id[]" value="<?= (!empty($reqItem->fgitem_id)) ? $reqItem->fgitem_id : '' ?>">
    																<input type="hidden" name="fgitem_name[]" value="<?=(!empty($reqItem->fgitem_name)) ? htmlentities($reqItem->fgitem_name) : ''?>">
																</td>
																<td>
																	<?= $reqItem->price ?>
																...
																<td class="text-center" style="width:10%;">
																	...
																	$reqItem->trans_id = "";
																	$reqItem->item_id = $reqItem->req_item_id;
																	$reqItem->qty = $reqItem->req_qty;
												...
												?>
														<tr>
															...
															<td>
																...
																<input type="hidden" name="fgitem_id[]" value="<?= (!empty($row->fgitem_id)) ? $row->fgitem_id : '' ?>">
																<input type="hidden" name="fgitem_name[]" value="<?=(!empty($row->fgitem_name)) ? htmlentities($row->fgitem_name) : ''?>">
															</td>
															<td>
																<?= $row->price ?>
															...
															<td class="text-center" style="width:10%;">
																<?php
    																...
    																if(empty($row->order_status) AND $row->order_status == 0):
    																$row = json_encode($row);
																?>
                    ...
                    <div class="card-footer">
                        <div class="col-md-12">
                            ...
                            <a href="<?=(!empty($order_type)?base_url($headData->controller.'/rmIndex'):base_url($headData->controller))?>" class="btn waves-effect waves-light btn-outline-secondary float-right save-form" style="margin-right:10px;"><i class="fa fa-times"></i> Cancel</a>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="col-md-12">
                        <div class="row form-group">
								...
								<select id="item_type" class="form-control single-select">
									...
									<?php
										foreach ($itemTypeList as $row) :
										    $disabled = (empty($order_type) AND $row->id == 3)?'disabled':'';
											echo '<option value="'.$row->category_type.'" '.$disabled.'>' . $row->category_name . '</option>';
										endforeach;
									?>
								</select>
							</div>
                            <div class="col-md-6 form-group">
                                ...
                                <select name="item_id" id="item_id" class="form-control large-select2 req" data-item_type="<?=(!empty($order_type)?'3':'1,2,4,5,6,7,8')?>" data-category_id="" data-family_id="" autocomplete="off" data-default_id="<?= (!empty($dataRow->item_id)) ? $dataRow->item_id : "" ?>" data-default_text="<?= (!empty($dataRow->full_name)) ? $dataRow->full_name : "" ?>" data-url="items/getDynamicItemList">
