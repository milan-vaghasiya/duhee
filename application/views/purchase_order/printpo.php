...
<div class="row">
	<div class="col-12">
		<table class="table">
		    <tr>
		        <td class="fs-16 text-left" style="width:30%;font-weight:bold;padding:0px !important;">Doc. No. : F/AD/PO/00</td>
		...
		<table class="table top-table-border" style="margin-top:2px;">
			...
			<tr>
				<th>Offer No.</th><td><?=$poData->quotation_no?></td>
			</tr>
			<tr>
				<th>Offer Date</th><td><?=(!empty($poData->quotation_date)) ? formatDate($poData->quotation_date) : ""?></td>
			</tr>
		</table>
		...
		<table class="table item-list-bb" style="margin-top:10px;">
				...
				if(!empty($poData->itemData)):
					foreach($poData->itemData as $row):
						...
						echo '<tr>';
							...
							echo '<td>'.$row->full_name.$indent.$delivery_date.'</td>';
			...
			<tr>
				<th colspan="5" class="text-left">Note : <?=$poData->remark?></th>
				<th colspan="2" class="text-right">P & F</th>
				<td class="text-right"><?=sprintf('%.2f',$poData->packing_charge)?></td>
			</tr>
			<tr>
				<th colspan="5" class="text-left" rowspan="<?=$rwspan?>">Amount In Words : <?=numToWordEnglish($poData->net_amount)?></th>
