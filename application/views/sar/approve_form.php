            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header text-center">
                        <h4><u>Approve / Reject Report</u></h4>
                    </div>
                    <div class="card-body">
                        <form id="approveForm">
                            <div class="col-md-12">
                                <div class="row">
                                    ...
                                    <div class="col-md-10 form-group">
                                        <label for="prod_remark">Remark</label>
                                        <input type="text" name="prod_remark" id="prod_remark" class="form-control" value="<?=(!empty($dataRow->prod_remark))?$dataRow->prod_remark:""?>" >
                                    </div>
                                </div>
                                <hr>
                                <div class="row">
                                                ...
                                                if (!empty($paramData)) :
                                                    foreach ($paramData as $row) :
                                                        ...
                                                        echo '<tr>
                                                                ...
                                                                for ($c = 0; $c < 5; $c++) :
                                                                    if (!empty($obj->{$row->id})) :
                                                                        echo '<td><input type="text" name="sample'.($c + 1).'_'.$row->id.'" id="sample'.($c + 1).'_'.$i.'" class="form-control text-center parameter_limit'.$cls.'" value="'.$obj->{$row->id}[$c].'" data-min="'.$row->min_req.'" data-max="'.$row->max_req.'" data-requirement="'.$row->requirement.'" data-row_id="'.$i.'"></td>';
                                                                    else :
                                                                        echo '<td><input type="text" name="sample'.($c + 1).'_'.$row->id.'" id="sample'.($c + 1).'_'.$i.'" class="form-control text-center parameter_limit'.$cls.'" value="" data-min="'.$row->min_req.'" data-max="'.$row->max_req.'" data-requirement="'.$row->requirement.'" data-row_id="'.$i.'"></td>';
                                                                    endif;
                                                                endfor;
                                                        echo '</tr>';
                                                    endforeach;
                                                else:
                                                    echo '<tr class="text-center"><td colspan="12">Data not available.</td></tr>';
                                                endif;
                                                $tbcnt++;
                                                ?>
                                            </tbody>
                    ...
                    <div class="card-footer">
                        <div class="col-md-12">
                            <button type="button" class="btn waves-effect waves-light btn-outline-success float-right save-form" onclick="saveApproveSar('approveForm');" ><i class="fa fa-check"></i> Save</button>
...
<script>
function saveApproveSar(formId){
	var fd = $('#'+formId)[0];
    var formData = new FormData(fd);
	$.ajax({
		url: base_url + controller + '/saveApproveSar',
		data:formData,
        processData: false,
        contentType: false,
		type: "POST",
