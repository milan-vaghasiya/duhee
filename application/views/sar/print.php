	...
	<div class="col-12">
		...
		<table class="table item-list-bb text-left">
			<tr>
				...
				<td style="width:30%"><?= (!empty($dataRow->full_name) ? $dataRow->full_name : '') ?></td>
				<th style="width:20%">Date</th>
				<td style="width:30%"><?= (!empty($dataRow->trans_date) ? formatDate($dataRow->trans_date) : '') ?></td>
			</tr>
			<tr>
				<th>Part Number</th>
			...
			<tr>
				...
				<th>M/C No.</th>
				<td><?= (!empty($dataRow->machine_code) ? $dataRow->machine_code : '') ?></td>
			</tr>
			<tr>
				<th>Lot No. / DT.</th>
			...
			<tr>
				...
				<td><?= (!empty($dataRow->cycle_time) ? $dataRow->cycle_time : '') ?></td>
			</tr>
			<tr>
				<th>GRN No.</th>
				<td></td>
				<th>Heat No.</th>
				<td><?= (!empty($dataRow->heat_no) ? $dataRow->heat_no : '') ?></td>
			</tr>
			<tr>
				<th>Drawing No.</th>
			...
			<tr>
				<th>Dept. Name</th>
				<td><?= (!empty($dataRow->name) ? $dataRow->name : '') ?></td>
				<th>Process</th>
				<td><?= (!empty($dataRow->process_name) ? $dataRow->process_name : '') ?></td>
			</tr>
		</table>
			...
			<tbody>
				...
				if (!empty($prodParamData)):
					foreach ($prodParamData as $param) :
						...
						$obj = new StdClass;
						$obj = json_decode($dataRow->prod_observation);
