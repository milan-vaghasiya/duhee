...
class DebitNote extends MY_Controller
{
	...
	public function getPurchaseInvoiceList(){
		...
		foreach ($purInvData as $row) {
			$piTransData=$this->purchaseInvoice->purchaseTransactions($row->id);
			$i=0;
			$item_ids='';
			foreach($piTransData as $pi){
				if($i==0){
					$item_ids.=$pi->item_id;
				}
				else{
			...
			$options .= '<option value="' . $row->id . '" '.$selected.' data-item_ids="'.$item_ids.'">' . $row->doc_no . '</option>';
		}
	...
	public function getInvoiceItem(){
		...
		if (!empty($postData['invoice_ids'])) :
			$htmlOptions = $this->debitNote->getInvoiceItemsForDebitNote($postData);
