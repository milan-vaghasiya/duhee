...
class Employees extends MY_Controller
{
    ...
    private $docTypeArr = [1=>"Pan Card",2=>"Aadhar Card",3=>"Election Card",4=>"Driving Licence",5=>"Bank Acc. Passbook",6=>"Leaving Certificate"];
	...
	public function __construct(){
		...
		$this->data['daysList'] = ['Sun'=>'Sunday','Mon'=>'Monday','Tue'=>'Tuesday','Wed'=>'Wednesday','Thu'=>'Thursday','Fri'=>'Friday','Sat'=>'Saturday'];
		$this->data['salaryFormat'] = ['1'=>'CTC (COST TO COMPANY)','2'=>'DAILY WAGES','3'=>'GROSS (Month)'];
	}
    ...
    public function save(){
        ...
        if(!empty($errorMessage)):
            ...
            if(empty($data['id'])){ $data['emp_role'] = 6; }
    ...
    public function updateEmpNom(){
		...
		if(!empty($errorMessage)):
            ...
            $nomData = $this->employee->getNominationData($data['emp_id']);
            $tbodyData="";$i=1; 
            if(!empty($nomData)):
                $i=1;
                foreach($nomData as $row):
                    $tbodyData.= '<tr>
                                ...
                                <td class="text-center">'.$row->nom_name.'</td>
                                <td class="text-center">'.$row->nom_gender.'</td>
                                <td class="text-center">'.$row->nom_relation.'</td>
                                <td class="text-center">'.formatDate($row->nom_dob).'</td>
                                <td class="text-center">'.$row->nom_proportion.'</td>
    ...
    public function updateEmpEdu(){
		...
		if(!empty($errorMessage)):
            ...
            $eduData = $this->employee->getEducationData($data['emp_id']);
            $tbodyData="";$i=1; 
            if(!empty($eduData)):
                $i=1;
                foreach($eduData as $row):
                    $tbodyData.= '<tr>
                                ...
                                <td class="text-center">'.$row->passing_year.'</td>
    ...
    public function empProfile($emp_id){
        ...
        $this->data['docTypeArr'] = $this->docTypeArr;
        $this->data['deptRows'] = $this->department->getDepartmentList();
        $this->data['descRows'] = $this->employee->getDesignation();
        $this->data['systemDesignation'] = $this->systemDesignation;
        $this->data['categoryData'] = $this->category->getCategoryList();
            ...
            if(!empty($docData)):
                ...
                foreach($docData as $row):
                    $tbodyData.= '<tr>
                                ...
                                <td class="text-center">'.$this->docTypeArr[$row->doc_type].'</td>
    ...
    ****/
    ...
    public function saveEmpSalaryStructure(){
		...
		if(empty($postData['sal_format']))
		{
			$errorMessage['sal_format'] = "Format is required.";
		}
		else
		{
			if($postData['sal_format'] == 1)
			{
				if(empty($postData['sal_amount'])){$errorMessage['sal_amount'] = "CTC Amount is required.";}
			}
			if($postData['sal_format'] == 2)
			{
				if(empty($postData['rate_hour'])){$errorMessage['rate_hour'] = "Rate/Hour is required.";}
				if(empty($postData['wh_day'])){$errorMessage['wh_day'] = "Hours/Day is required.";}
			}
		}
        ...
        if(!empty($errorMessage)):
			...
			$postData['emp_id'] = $postData['ctc_emp_id'];
			$postData['emp_code'] = $postData['ctc_emp_code'];
			$postData['esic_status'] = $postData['ctc_esic_status'];
			$postData['pf_status'] = $postData['ctc_pf_applicable'];
			$postData['id'] = $postData['sal_id'];
			unset($postData['ctc_emp_id'],$postData['ctc_emp_code'],$postData['sal_id'],$postData['ctc_esic_status'],$postData['ctc_pf_applicable']);
			//print_r($postData);exit;
            $this->printJson($this->employee->saveEmpSalaryStructure($postData));
        endif;
    }
    ...
    public function getEmpActiveSalaryStructure(){
        ...
        if(!empty($postData['emp_id']))
		{
			...
			$actStr = $this->employee->getActiveSalaryStructure($postData['emp_id']);
			if(empty($actStr))
			{
				$salConfig = $this->employee->getMasterOptions();
			}
			else
			{
				$oldStructureId = $actStr->id;
				$salConfig->pf_per = $actStr->pf_per;
				$salConfig->emp_esic_per = $actStr->emp_esic_per;
				$salConfig->cmp_esic_per = $actStr->cmp_esic_per;
				$salConfig->gratuity_per = $actStr->gratuity_per;
				$salConfig->bonus_per = $actStr->bonus_per;
			}
			$salConfig->basic_per = $postData['basic_per'];
			$salConfig->hra_per = $postData['hra_per'];
			$salConfig->pf_limit = $postData['pf_limit'];
			$salConfig->pf_on = $postData['pf_on'];
			$salConfig->pt_limit = 12000; // IF GROSS SALARY > 12000 THEN 200 ELSE 0
			
			$sal_id = (!empty($actStr))?$actStr->id:'';
			$transport_charge = (!empty($actStr)) ? $actStr->transport_charge : $postData['transport_charge'];			
			$pf_applicable = $postData['ctc_pf_applicable'];
			$esic_status = $postData['ctc_esic_status'];
			$food = 0;$tds = 0;$gratuity = 0;$monthDay = 26;$bonus_day_hrs=8;
						
			if($postData['sal_format'] == 1)
			{
				$bonus = (!empty($actStr)) ? $actStr->bonus : $postData['bonus'];
				$tds = (!empty($actStr)) ? $actStr->tds : $postData['tds'];
				$sal_amount = (!empty($actStr)) ? $actStr->sal_amount : $postData['sal_amount'];
				
				$other_all = 0;$emp_pf = 0;$emp_esic = 0;$cmp_esic = 0;
				
				$basic_salary = round((($sal_amount*$salConfig->basic_per)/100),0);
				$hra_amt = round((($basic_salary * $salConfig->hra_per)/100),0);
				$gross_sal = (!empty($postData['gross_sal'])) ? $postData['gross_sal'] : 0;
				$other_all = $basic_salary - $hra_amt;
				...
				{
					$pfCalcOn = (($basic_salary + $other_all) < $salConfig->pf_limit) ? ($basic_salary + $other_all) : $salConfig->pf_limit;
				}
				if($pf_applicable == 1){$emp_pf = round((($pfCalcOn * $salConfig->pf_per)/100),0);}
				
				// Calc ESIC
				if(!empty($esic_status)){$emp_esic = round((($gross_sal*$salConfig->emp_esic_per)/100),0);}
				if(!empty($esic_status)){$cmp_esic = round((($gross_sal*$salConfig->cmp_esic_per)/100),0);}
				
				$pt = ($gross_sal > $salConfig->pt_limit)  ? 200 : 0;
				$total_ded = $emp_pf + $emp_esic + $pt;
				$take_home = $gross_sal - $total_ded + $bonus;
				$bonus = 1800;
				$bonus = round((($rate_hour * $bonus_day_hrs * $monthDay * $salConfig->bonus_per)/100));
				$gratuity = (!empty($salConfig->gratuity)) ? round((($basic_salary*$salConfig->gratuity)/100),0) : 0;
				$cmp_pf = $emp_pf;
				$total_benefit = $bonus + $gratuity + $cmp_pf;
				
				$take_home = $gross_sal - $total_ded + $bonus;
				
				$ctc_month = $gross_sal + $total_benefit;
				
				$strData = '<div class="table table-responsive">
							<table class="table table-bordered align-items-center mt-2">
								<tr style="background:#00CCFF;"><th>Description</th><th>Per Month</th><th>Per Annum</th></tr>
								<tr><td>Basic</td><td id="basic_sal_m">'.$basic_salary.'</td><td id="basic_sal_y">'.($basic_salary*12).'</td></tr>
								<tr><td>HRA</td><td id="hra_m">'.$hra_amt.'</td><td id="hra_y">'.($hra_amt*12).'</td></tr>
								<tr><td>Other All</td><td id="other_m">'.$other_all.'</td><td id="other_y">'.($other_all*12).'</td></tr>
								<tr style="background:#C4BD97"><th>Gross (I)</th><th id="gross_m">'.$gross_sal.'</th><th id="gross_y">'.($gross_sal*12).'</th></tr>
								<tr><td>PF</td><td id="pf_m">'.$emp_pf.'</td><td id="pf_y">'.($emp_pf*12).'</td></tr>
								<tr><td>ESIC</td><td id="esic_m">'.$emp_esic.'</td><td id="esic_y">'.($emp_esic*12).'</td></tr>
								<tr><td>PT</td><td id="pt_m">'.$pt.'</td><td id="pt_y">'.($pt*12).'</td></tr>
								<tr style="background:#DFDFDF">
									<th>Total Deduction (II)</th><th id="total_ded_m">'.$total_ded.'</th><th id="total_ded_y">'.($total_ded*12).'</th>
								</tr>
								<tr><td>BONUS ADV.</td><td id="bonus_m">'.$bonus.'</td><td id="bonus_y">'.($bonus*12).'</td></tr>
								<tr style="background:#FFF000">
									<th>Take Home(NET) (I-II)</th><th id="take_home_m">'.$take_home.'</th><th id="take_home_y">'.($take_home*12).'</th>
								</tr>
								<tr><td>Bonus</td><td id="bonus_m">'.$bonus.'</td><td id="bonus_y">'.($bonus*12).'</td></tr>
								<tr><td>Gratuity(4.81%)</td><td id="gratuity_m">'.$gratuity.'</td><td id="gratuity_y">'.($gratuity*12).'</td></tr>
								<tr><td>PF (Company)</td><td id="pf_cmp_m">'.$cmp_pf.'</td><td id="pf_cmp_y">'.($cmp_pf*12).'</td></tr>
								<tr class="bg-light">
									<th>Total Annual benefit (III)</th><th id="total_benefit_m">'.$total_benefit.'</th><th id="total_benefit_y">'.($total_benefit*12).'</th>
								</tr>
								<tr style="background:#09D091">
									<th>CTC(P.M) (I+III)</th><th id="sal_amount_m">'.$ctc_month.'</th><th id="sal_amount_y">'.($ctc_month*12).'</th>
								</tr>
							</table>
						</div>';
				// Hidden Data
				$strData .= '<input type="hidden" name="sal_id" value="'.$sal_id.'">';
				$strData .= '<input type="hidden" name="basic_salary" value="'.$basic_salary.'" />';
				$strData .= '<input type="hidden" name="basic_per" value="'.$salConfig->basic_per.'" />';
				$strData .= '<input type="hidden" name="hra_amt" value="'.$hra_amt.'" />';
				$strData .= '<input type="hidden" name="other_all" value="'.$other_all.'" />';
				$strData .= '<input type="hidden" name="emp_pf" value="'.$emp_pf.'" />';
				$strData .= '<input type="hidden" name="pt" value="'.$pt.'" />';
				$strData .= '<input type="hidden" name="pf_per" value="'.$salConfig->pf_per.'" />';
				$strData .= '<input type="hidden" name="emp_esic" value="'.$emp_esic.'" />';
				$strData .= '<input type="hidden" name="emp_esic_per" value="'.$salConfig->emp_esic_per.'" />';
				$strData .= '<input type="hidden" name="food" value="'.$food.'" />';
				$strData .= '<input type="hidden" name="cmp_esic_per" value="'.$salConfig->cmp_esic_per.'" />';
				$strData .= '<input type="hidden" name="cmp_esic" value="'.$cmp_esic.'" />';
				$strData .= '<input type="hidden" name="cmp_pf" value="'.$cmp_pf.'" />';
				$strData .= '<input type="hidden" name="bonus" value="'.$bonus.'" />';
				$strData .= '<input type="hidden" name="bonus_per" value="'.$salConfig->bonus_per.'" />';
				$strData .= '<input type="hidden" name="gratuity" value="'.$gratuity.'" />';
				$strData .= '<input type="hidden" name="gratuity_per" value="'.$salConfig->gratuity_per.'" />';
			}
			if($postData['sal_format'] == 2)
			{
				$bonus = 0;$other_all = 0;$emp_pf = 0;$emp_esic = 0;$cmp_esic = 0;$gratuity = 0;$hra_per = (!empty($postData['hra_per'])) ? $postData['hra_per'] : 0;
				$anum = 12;$thTitle = '<th>Per Month</th><th>Per Annum</th>';
				
				$rate_hour = $postData['rate_hour']; //(!empty($actStr)) ? $actStr->rate_hour : $postData['rate_hour'];
				$wh_day = $postData['wh_day']; //(!empty($actStr)) ? $actStr->wh_day : $postData['wh_day'];
				$sal_amount = round(($rate_hour * $wh_day * $monthDay));
				
				$bonus = round((($rate_hour * $bonus_day_hrs * $monthDay * $salConfig->bonus_per)/100));
				
				$basic_salary = round((($sal_amount*$salConfig->basic_per)/100));
				$hra_amt = $sal_amount - $basic_salary; // round((($basic_salary * $hra_per)/100));				
				$gross_sal = $basic_salary + $hra_amt;
				...
				// Calc ESIC
				if(!empty($esic_status)){$emp_esic = round((($gross_sal*$salConfig->emp_esic_per)/100));}
				if(!empty($esic_status)){$cmp_esic = round((($gross_sal*$salConfig->cmp_esic_per)/100));}
				
				$gratuity = (!empty($salConfig->gratuity_per)) ? round((($basic_salary*$salConfig->gratuity_per)/100)) : 0;
				
				$pt = ($gross_sal > $salConfig->pt_limit)  ? 200 : 0;
				$total_ded = $emp_pf + $emp_esic + $pt;
				$total_benefit = $gratuity + $cmp_pf + $bonus;
				$take_home = $gross_sal - $total_ded;
				$ctc_month = $gross_sal + $total_benefit;
				
				$strData = '<div class="table table-responsive">
							<table class="table table-bordered align-items-center mt-2">
								<tr style="background:#00CCFF;"><th>Description</th>'.$thTitle.'</tr>
								<tr><td>Basic</td><td id="basic_sal_m">'.$basic_salary.'</td><td id="basic_sal_y">'.($basic_salary*30).'</td></tr>
								<tr><td>HRA</td><td id="hra_m">'.$hra_amt.'</td><td id="hra_y">'.($hra_amt*30).'</td></tr>
								<tr style="background:#C4BD97"><th>Gross (I)</th><th id="gross_m">'.$gross_sal.'</th><th id="gross_y">'.($gross_sal*30).'</th></tr>
								<tr><td>PF</td><td id="pf_m">'.$emp_pf.'</td><td id="pf_y">'.($emp_pf*30).'</td></tr>
								<tr><td>ESIC</td><td id="esic_m">'.$emp_esic.'</td><td id="esic_y">'.($emp_esic*30).'</td></tr>
								<tr><td>PT</td><td id="pt_m">'.$pt.'</td><td id="pt_y">'.($pt*30).'</td></tr>
								<tr style="background:#DFDFDF">
									<th>Total Deduction (II)</th><th id="total_ded_m">'.$total_ded.'</th><th id="total_ded_y">'.($total_ded*30).'</th>
								</tr>
								<tr style="background:#FFF000">
									<th>Take Home(NET) (I-II)</th><th id="take_home_m">'.$take_home.'</th><th id="take_home_y">'.($take_home*30).'</th>
								</tr>
								<tr><td>PF (Company)</td><td id="pf_cmp_m">'.$cmp_pf.'</td><td id="pf_cmp_y">'.($cmp_pf*30).'</td></tr>
								<tr><td>Bonus</td><td id="bonus_m">'.$bonus.'</td><td id="hra_y">'.($bonus*30).'</td></tr>
								<tr class="bg-light">
									<th>Total Annual benefit (III)</th><th id="total_benefit_m">'.$total_benefit.'</th><th id="total_benefit_y">'.($total_benefit*30).'</th>
								</tr>
								<tr style="background:#09D091">
									<th>CTC (I+III)</th><th id="sal_amount_m">'.$ctc_month.'</th><th id="sal_amount_y">'.($ctc_month*30).'</th>
								</tr>
							</table>
						</div>';
		...
		$trashBtn = '<div class="col-md-9"></div>';
		if(!empty($oldStructureId) AND $oldStructureId >0 )
		{
			$trashBtn = '<div class="col-md-6"></div>
						<div class="col-md-3">
							<button type="button" class="btn btn-danger btn-delete btn-block float-right" onclick="trashEmpSalStructure('.$oldStructureId.');"><i class="fa fa-bin"></i> Remove</button>
						</div>';
		}
		$saveParam = "'empCTCStructure','saveEmpSalaryStructure'";
		$strData .= '<div class="row">
						'.$trashBtn.'
						<div class="col-md-3">
							<button type="button" class="btn btn-success btn-save btn-block float-right" onclick="saveEmpSalaryStructure('.$saveParam.');"><i class="fa fa-check"></i> Save</button>
						</div>
					</div>';
        $this->printJson(['html'=>$html.$strData,'strData'=>$strData]);
    }

    // Delete/Trash Old Emp Structure Created By JP @ 12.07.2023
    public function trashEmpSalStructure(){
        $postData=$this->input->post();
        if(empty($postData['id'])):
            $this->printJson(['status'=>0,'message'=>'Somthing went wrong...Please try again.']);
        else:
            $this->printJson($this->employee->trashEmpSalStructure($postData['id']));
        endif;
    }
}
?>