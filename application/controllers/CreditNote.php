...
class CreditNote extends MY_Controller{	
	...
	public function getSalesInvoiceList()
	{
		...
		foreach ($purInvData as $row) {
			$siTransData=$this->salesInvoice->salesTransactions($row->id);
			$i=0;
			$item_ids='';
			foreach($siTransData as $si){
				if($i==0){
					$item_ids.=$si->item_id;
				}
				else{
			...
			$options .= '<option value="' . $row->id . '" '.$selected.'  data-item_ids="'.$item_ids.'">' . getPrefixNumber($row->trans_prefix,$row->trans_no) . '</option>';
		}
		$this->printJson(['status' => 1, 'options' => $options]);
	}
	...
	public function getInvoiceItem()
	{
		...
		$postData = $this->input->post();
		$htmlOptions="";
		if (!empty($postData['invoice_ids'])) :
			$htmlOptions = $this->creditNote->getInvoiceItemsForCreditNote($postData);
		else:
