...
class FinalCFT extends MY_Controller
{
    ...
    public function convertToScrap(){
        $data = $this->input->post();
        $this->printJson($this->finalCFT->convertToScrap($data));
    }

    public function printTag($tag_type,$id){
        $tagData = $this->primaryCFT->getTagData($id); 
        $firData = $this->finalCFT->getFIRData($tagData->job_trans_id);
        ...
        $vendorName = (!empty($tagData->rej_by_name) && $tagData->mfg_by == 2) ? $tagData->rej_by_name : $tagData->operator_name;
        ...
        $itemList = '<table class="table table-bordered vendor_challan_table">
            <tr>
                ...
                <td style="font-size:0.7rem;width:20%" class="text-center"><b>Batch No</b></td>
            ...
            <tr>
				...
				<td style="font-size:0.7rem;" class="text-center">' . $firData->fg_batch_no . '</td>
            ...
            <tr>
                ...
                <td style="font-size:0.7rem;" colspan="4">' . $tagData->operator_name . '</td>
            </tr>
		</table>';
