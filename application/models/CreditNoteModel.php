...
class CreditNoteModel extends MasterModel
{
    ...
    public function getInvoiceItemsForCreditNote($postData)
    {
        $data['tableName'] = $this->transChild;
        $data['select'] = "trans_child.*,AVG(price) as avg_price";
        $data['where_in']['trans_child.trans_main_id'] = $postData['invoice_ids'];
        ...
        if (!empty($itemData)) :
            foreach ($itemData as $row) :
                $itmName = (!empty($row->drawing_no)) ? $row->item_name . "(" . $row->drawing_no . ")" : $row->item_name;
