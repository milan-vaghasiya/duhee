	...
	class BiometricModel extends MasterModel{
				...
				foreach ($deviceData as $row) :
					...
					if (!empty($punchData)) {
						foreach ($punchData as $punch) {
							...
							$device_id = (!empty($punch->mcid)) ? $punch->mcid : $row->id;
		...
		public function getEmpPunchesByDate($empId,$currentDate)
		{
			...
			CAST(DATE_SUB(CONCAT('".$currentDate."', ' ', shift_master.shift_start), INTERVAL 7200 SECOND) as datetime) as shiftStart,
		...
		public function getAllPunches($postData = [])
		{
			...
			CAST(DATE_SUB(CONCAT('".$report_date."', ' ', shift_master.shift_start), INTERVAL 7200 SECOND) as datetime) as shiftStart,
			CAST(DATE_ADD(CONCAT('".$report_date."', ' ', shift_master.shift_start), INTERVAL 79199 SECOND) as datetime) as shiftEnd,
			(
		...
		public function getPunchByDate($currentDate)
		{
			...
			LEFT JOIN department_master ON department_master.id = employee_master.emp_dept_id
			LEFT JOIN emp_designation ON emp_designation.id = employee_master.emp_designation
			WHERE MONTH(emp_shiftlog.month) = ".$month." AND YEAR(emp_shiftlog.month) = ".$year." AND employee_master.is_active=1 GROUP BY emp_shiftlog.emp_id ORDER BY employee_master.emp_code")->result();
			//$this->printQuery();
			return $attendanceData;
		}
	...
	// 		$queryData['where']['device_id'] = $device_id;
		...
		public function callDeviceApi($deviceData)
		{
			...
			if(isset($deviceData->MCID) AND $deviceData->MCID==1)
			{
			    $api_url = "https://api.etimeoffice.com/api/DownloadPunchDataMCID?Empcode=".$deviceData->Empcode."&FromDate=".$deviceData->FromDate."&ToDate=".$deviceData->ToDate;
			}
		...
		public function arrangeAttendanceDate($empId,$currentDate)
		{
			...
			CAST(DATE_SUB(CONCAT('".$currentDate."', ' ', shift_master.shift_start), INTERVAL 7200 SECOND) as datetime) as shiftStart,emp.emp_code,
			CAST(DATE_ADD(CONCAT('".$currentDate."', ' ', shift_master.shift_start), INTERVAL 79199 SECOND) as datetime) as shiftEnd,
			(
				SELECT GROUP_CONCAT(al.punch_date) FROM attendance_log as al WHERE al.emp_id = ".$empId." AND al.punch_type IN (1,2) AND al.punch_date >= shiftStart AND al.punch_date <= shiftEnd AND al.is_delete=0
			) as punch_date, 
		...
		/*** Created By : JP@28.08.2023 ***/
		public function getFirstInPunch($postData)
		{
			$aData['tableName'] = $this->attendanceLog;
			$aData['select'] = "attendance_log.id,attendance_log.punch_date";
			$aData['where_in']['attendance_log.device_id'] = [1,3];
			$aData['where']['attendance_log.punch_type != '] = 3;
			$aData['where']['attendance_log.punch_date >= '] = date('Y-m-d 00:00:00',strtotime($postData['from_date']));
			$aData['where']['attendance_log.punch_date <= '] = date('Y-m-d 23:59:59',strtotime($postData['from_date']));
			$aData['where']['attendance_log.emp_id'] = $postData['emp_id'];
			$aData['order_by']['attendance_log.punch_date'] = 'ASC';
			$aData['limit']= 1;
			$attendanceData = $this->row($aData);
			//$this->printQuery();
			return $attendanceData;
		}
		
		/*** Created By : JP@28.08.2023 ***/
		public function getInOutPunches($postData)
		{
			$aData['tableName'] = $this->attendanceLog;
			$aData['select'] = "attendance_log.id, attendance_log.punch_type , attendance_log.device_id, attendance_log.punch_date, attendance_log.xtype, attendance_log.ex_hours, attendance_log.ex_mins";
			//$aData['leftJoin']['employee_master emp'] = "emp.id=attendance_log.emp_id";
			//$aData['leftJoin']['department_master dept'] = "emp.emp_dept_id = dept.id";
			$aData['where']['attendance_log.punch_date >= '] = $postData['from_date'];
			$aData['where']['attendance_log.punch_date <= '] = $postData['to_date'];
			if(!empty($postData['emp_code'])){$aData['where']['attendance_log.emp_code'] = $postData['emp_code'];}
			if(!empty($postData['emp_id'])){$aData['where']['attendance_log.emp_id'] = $postData['emp_id'];}
			$aData['order_by']['attendance_log.emp_code'] = 'ASC';
			$aData['order_by']['attendance_log.punch_date'] = 'ASC';
			$attendanceData = $this->rows($aData);
			//$this->printQuery();
			return $attendanceData;
		...
		public function getPunchLog($paramData = []){
			...
			{
				$begin = new DateTime($paramData['from_date']);
				$end = new DateTime($paramData['to_date']);
				$end = $end->modify( '+1 day' ); 
				
				$interval = new DateInterval('P1D');
				$dateRange = new DatePeriod($begin, $interval ,$end);
				
				$paramData['biomatric_id'] = (!empty($paramData['biomatric_id'])) ? $paramData['biomatric_id'] : '' ;
				$paramData['biomatric_id'] = ($paramData['biomatric_id'] == "ALL") ? '' : $paramData['biomatric_id'] ;
				$totalDays = intval(date('t',strtotime($paramData['from_date'])));
				$nightStart = 16;$nightEnd = '15:59:59';
				$empData = $this->employee->getEmpListForAttendance($paramData);
				...
				foreach($empData as $row)
				{
					...
					$punchData['total_days'] = $totalDays;
					$punchData['week_off'] = 0;
					$punchData['present'] = 0;
					$punchData['absent'] = 0;
					$punchData['twh'] = 0;
					$punchData['tot'] = 0;
					$punchData['wh'] = 0;
					$punchData['allPunches'] = '';
					$punchData['punchCount'] = 0;
					$stay_time=0;$twh = 0;$wh=0;$currentDay='';
					foreach($dateRange as $date)
					{
						...
						$empAttendanceLog = Array();$empPunches = Array();$logArr = Array();$allPunches = '';
						$row->shift_name = 'Day Shift';$t=1;$wph = Array();$idx=0;$stay_time=0;$twh = 0;$wh=0;
						...
						if(!empty($fInLog))
						{
							...
							if(!empty($empAttendanceLog))
							{
								...
								{
									$empPunches[] = $aLog->punch_date;
									if($aLog->punch_type==3){$row->ex_time += ($aLog->ex_hours * 3600) + ($aLog->ex_mins * 60);}
								}
							}
						...
						if(!empty($empPunches))
						{
							...
							// Set Present/Absent/ Status
							if($currentDay == 'Wed'){if($twh >= 3600){$punchData['present']++;}else{$punchData['week_off']++;}}
							else{ if($twh < 3600){$punchData['absent']++;}else{$punchData['present']++;} }
						}
						else
						{
							if($currentDay == 'Wed'){$punchData['week_off']++;}else{$punchData['absent']++;}
						}
						$punchData['twh'] += $twh;
						$punchData['tot'] = $row->over_time;
					}
					$punchData['wh'] = $punchData['twh'] - $punchData['tot'];
					$punchLogs[] = $punchData;
				}
