...
function getHourlyReport(file_type='view')
{
	...
	if(file_type == 'view')
	{
		$.ajax({
			...
			type:'post',
			data:{month:month,dept_id:dept_id,file_type:file_type,emp_type:emp_type,report_type:report_type,biomatric_id:biomatric_id},
			dataType:'json',
			success:function(data){
	...
	{
		window.open(base_url + controller + '/getHourlyReport/' + month + '/' + dept_id + '/' + emp_type + '/' + report_type + '/' + file_type + '/' + biomatric_id, '_blank').focus();
	}
}
...
function printaLogSummary(file_type)
{
    var month = $("#month").val();
    var from_date = $("#from_date").val();
    var to_date = $("#to_date").val();
    var biomatric_id = $('#biomatric_id').val();
    var fnName = '/getPrintaLogSummary/';
	window.open(base_url + controller + fnName +from_date+'~'+to_date+'/'+biomatric_id+'/'+file_type, '_blank').focus();
}
