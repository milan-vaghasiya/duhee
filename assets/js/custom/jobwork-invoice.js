$(document).ready(function () {
	...
	$(document).on('click','#addExp',function(){
		...
		$.each($(".expCheck"),function(){

			if($(this).prop('checked') == true){

				map_code = $(this).data('map_code');

				$("#exp_tr_"+map_code).show();

			}else{

				map_code = $(this).data('map_code');

				$("#exp_tr_"+map_code).hide();
	...
	$(document).on('click', '.createJobWorkInvoice', function () {
		...
		if (party_id != "" || party_id != 0) {
			...
			$.ajax({
				...
				url: base_url + 'jobWorkInvoice/getVendorJobWork',
				...
				success: function (data) {
					...
					$("#createInvoice").html('Create JobWork Invoice');

					$("#party_so").attr('action', base_url + 'jobWorkInvoice/createInvoice');

					$("#btn-create").html('<i class="fa fa-check"></i> Create JobWork Invoice');

					$("#partyName").html(party_name);

					$(".modal-title").html("Job Work ["+party_name+"]");
...
function Remove(button) {
	...
	var table = $("#jobinvoiceItems")[0];

	table.deleteRow(row[0].rowIndex);

	$('#jobinvoiceItems tbody tr td:nth-child(1)').each(function (idx, ele) {

		ele.textContent = idx + 1;

	});

	var countTR = $('#jobinvoiceItems tbody tr:last').index() + 1;
...
function AddRow(data) {
	...
	$('table#jobinvoiceItems tr#noData').remove();

	//Get the reference of the Table's TBODY element.

	var tblName = "jobinvoiceItems";
	...
	var qtyInput = $("<input/>", { type: "text", name: "qty[]",id:"qty"+countRow,class:"form-control floatOnly",onkeyup:"calculateAmt("+JSON.stringify(data)+",this,"+countRow+")", value: data.qty });
	...
	var gstPerInput = $("<input/>", { type: "text", name: "gst_per[]", id:'gst_per'+countRow, class:"form-control floatOnly",onkeyup:"calculateAmt("+JSON.stringify(data)+",this,"+countRow+")", value: data.gst_per });
	cell = $(row.insertCell(-1));
	cell.html(gstPerInput);
...
function calculateAmt(data,button,rowIndex){
	var id = button.getAttribute('id');

	var qty= $("#qty"+rowIndex).val();
	var gst_per = $("#gst_per"+rowIndex).val();

	qty = (parseFloat(qty) > 0)?qty:'';
	gst_per = (parseFloat(gst_per) > 0)?gst_per:'';	

	var qtyVal = (parseFloat(qty) > 0)?qty:0;
	var gst_per_val = (parseFloat(gst_per) > 0)?gst_per:0;

	data.row_index = $(button).closest("tr").index().toString();	

	var amount = 0; var total = 0; var disc_amt = 0; var igst_amt = 0;
	var cgst_amt = 0; var sgst_amt = 0; var net_amount = 0; var cgst_per = 0; var sgst_per = 0; var igst_per = 0;
	if (data.disc_per == "" && data.disc_per == "0") {
		amount = parseFloat(parseFloat(qtyVal) * parseFloat(data.price)).toFixed(2);
	} else {
		total = parseFloat(parseFloat(qtyVal) * parseFloat(data.price)).toFixed(2);
		disc_amt = parseFloat((total * parseFloat(data.disc_per)) / 100).toFixed(2);
		amount = parseFloat(total - disc_amt).toFixed(2);
	}

	cgst_per = parseFloat(parseFloat(gst_per_val) / 2).toFixed(2);
	sgst_per = parseFloat(parseFloat(gst_per_val) / 2).toFixed(2);	

	cgst_amt = parseFloat((cgst_per * amount) / 100).toFixed(2);
	sgst_amt = parseFloat((sgst_per * amount) / 100).toFixed(2);

	igst_per = parseFloat(gst_per_val).toFixed(2);
	igst_amt = parseFloat((igst_per * amount) / 100).toFixed(2);

	net_amount = parseFloat(parseFloat(amount) + parseFloat(igst_amt)).toFixed(2);

	data.gst_type = $('#gst_type').val();
	data.qty = qty;
