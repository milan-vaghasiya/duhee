$(document).ready(function(){
	...
	$(document).on('change',"#material_used_id",function(){
		if($(this).val() != ""){
			...
			if(outQty=="" || outQty==null || isNaN(outQty)){$(".out_qty").html("Out Qty. is required.");}
			else
			{
...
function saveOutward(formId){
    ...
    $.ajax({
		...
		if(data.status===0){
			...
			$("#out_qty").val("");
			$("#in_qty_kg").val("");
			$("#setup_status").val(0);$("#setup_status").comboSelect();
			$("#setter_id").val("");$("#setter_id").comboSelect();
			$("#machine_id").val("");$("#machine_id").comboSelect();
			$("#material_used_id").val("");$("#material_used_id").comboSelect();
