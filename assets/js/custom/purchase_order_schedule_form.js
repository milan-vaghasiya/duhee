$(document).ready(function () {
    ...
    $(document).on('change load', '#ref_id', function () {
    ...
    $(document).on("change", "#item_id", function () {
        ...
        if (itemId == "") {
            ...
            $("#remarks").val(itemData.remark);

            console.log(itemData);

            $("#price").val("");

            var order_id = $("#ref_id").val();

            $.ajax({

                url: base_url + controller + '/getSheduleItemPrice',
...
$(document).on('click', '.saveItem', function () {
    ...
    if (formData.item_id == "") {
        ...
        /* if ($.inArray(formData.item_id, itemIds) >= 0 && formData.trans_id=='') {
...
function AddRow(data) {
    ...
    var remarksInput = $("<input/>", { type: "hidden", name: "remarks[]", value: data.remarks });
