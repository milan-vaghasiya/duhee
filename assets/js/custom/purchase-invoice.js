...
function gstType() {
	...
	if (gstApplicable == 1) {
		...
		if ($("#party_id").val() != "") {
			...
			if (gstin != "") {
				stateCode = gstin.substr(0, 2);
				if (stateCode == 24 || stateCode == "24") { gstType = 1; console.log("Gujarat"); } else { gstType = 2; }
			} else {
				gstType = 1;
			}
		} else {
		...
		$("#gst_type").val(gstType);
		console.log("GST Function"+$("#gst_type").val());
		if (gstType == 1) {
			$(".cgstCol").show(); $(".sgstCol").show(); $(".igstCol").hide();
			$(".amountCol").hide(); $(".netAmtCol").show(); $(".itemGst").show();
		} else if (gstType == 2) {
...
function claculateColumn() {
	...
	var amountSum = 0;var subTotal = 0;
	...
	$('#igst_amt_total').val("");
	$('#igst_amt_total').val(igstAmtSum.toFixed(2));$('.igst_amt_total').html($('#igst_amt_total').val());
...
function AddRow(data) {
	console.log("Add Row Function Call");
